...
- **Internal inconsistency across exports:** the same YE Jun 2026 year shows net domestic migration of +8,492 (O-D table sum), +11,647 (growth-breakdown file), and +12,398 (CBSA file). **Client confirmation (8/13/26): the +8,492 O-D sum is NOT the correct level** — consistent with the O-D table truncating small/unmatched flows (many rows show inbound>0, outbound exactly 0, indicating a minimum-count threshold). Use the O-D file for feeder mix and trend only. The level is +11,647 or +12,398 (growth file vs national CBSA model — which is authoritative within Placer's methodology remains open); treat as "~+11.6–12.4K" and the *trend* (near-doubling) as the robust finding.
