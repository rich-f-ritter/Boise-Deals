"""Write Tables/decisions_log.md + Tables/methodology.md for each subject (audit trail)."""
import sys
...
SUBJECTS = {
    "CanyonRidge": {
        "name": "Canyon Ridge", "addr": "2552 E Gowen Rd, Boise, ID 83716",
        "pt": "43.541734, -116.151198",
        "loc": ("Canyon Ridge is a 287-unit, 4-story apartment community completed 2024 on the "
                "south/southeast side of Boise Airport (Gowen Field), in the Columbia Village / "
                "Columbia Town Center area along E Gowen Rd. The verified point sits on the Columbia "
                "Town Center MX-2 node (~0.1 mi from the apartment footprint) — immaterial at a 5-mi "
                "radius. The subject sits INSIDE the Airport Influence Area, where new residential is "
                "restricted/prohibited in Zones B and C."),
        "span": ("Boise Airport (BOI)/Gowen Field; the Micron HQ + fab expansion on the Columbia bench; "
                 "the Gowen Rd / Federal Way / Eisenman / S Orchard industrial corridor (CCDC Gateway East "
                 "urban-renewal district); Columbia Village; the Parkcenter / Boise River / Barber Valley "
                 "edge; and unincorporated Ada County rural-urban frontier to the south and east."),
    },
    "SeasonsMeridian": {
        "name": "Seasons at Meridian", "addr": "2700 E Overland Rd, Meridian, ID 83642",
        "pt": "43.591935, -116.360877",
        "loc": ("Seasons at Meridian is an apartment community on E Overland Rd in southeast Meridian, "
                "~0.5 mi south of I-84 near S Eagle Rd, in the heart of the Treasure Valley's fastest-"
                "growing residential submarket. No airport overlay applies here."),
        "span": ("central and south Meridian; the Ten Mile Interchange specific-area plan; the Overland / "
                 "Eagle Rd / Locust Grove / Meridian Rd / Lake Hazel growth corridors; the Chinden / SH-20-26 "
                 "employment frontier; west Boise and a sliver of Garden City to the east; and unincorporated "
                 "Ada County (city Areas of Impact) at the fringes."),
    },
}


def log(s):
    return f"""# Land Use & Developable-Land Analysis — {s['name']} — Decisions Log
...
- Address: {s['addr']} — {s['name']}.
- Verified point: {s['pt']} (ArcGIS World GeocodeServer, PointAddress, score 100).
- {s['loc']}
...
def methodology(s):
    return f"""1. Subject resolution — geocoded {s['addr']} to {s['pt']} (verified against satellite + parcel).
...
8. Deliverables — this workbook (fully auditable), an interactive HTML map viewer (land use / zoning /
   vacant-threat), and a head-to-head developable-land comparison with {("Seasons at Meridian" if s['name']=="Canyon Ridge" else "Canyon Ridge")}.
"""
...
def main():
    for key, s in SUBJECTS.items():
        ...
        (tbl / "decisions_log.md").write_text(log(s), encoding="utf-8")
        (tbl / "methodology.md").write_text(methodology(s), encoding="utf-8")
        print(f"wrote {key}/Tables/decisions_log.md + methodology.md")
