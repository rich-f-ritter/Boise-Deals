...
2. **Shadow-supply scan (Part B).** Research the 5-mile radius for **latent supply
   ...
   **Start Part B at the subject itself (MANDATORY):** (a) check whether the
   subject is a phase of a larger plan — pull its subdivision/DA and groundbreaking
   press; if it's "Phase 1" of anything, the later phases are shadow-supply row #1;
   (b) sweep the subject's developer + architect for sibling/"II" projects within
   ~1 mi; (c) inventory the parcels adjacent to the subject for entitlements,
   searching by **address, parcel number, owner and subdivision name — never only
   the marketed project name** (city files routinely use a different name; a
   351-unit "Phase II" next door to one subject was once missed because it was
   filed as "<Subdivision> II" and coded as farmland). Vendor feeds will NOT
   contain these — that is the point of Part B.
